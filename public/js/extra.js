    ...
    //clear the terminal
      ...
      if (!weatherData) {
          ...
          (response) => {
            response.json().then((data) => {
              if (data.error) {
                terminal.insertAdjacentHTML(
                  "beforeend",
                  `<p class='textError'>>>>  ${data.error}</p>`
                );
              } else {
                weatherData = data;
                terminal.insertAdjacentHTML(
                  "beforeend",
                  `<p class='textReturn'>>>>  weather data loaded ! you can start your queries now</p>`
                );
              }
            });
          }
        ...
        if (weatherData.data[input]) {
          terminal.insertAdjacentHTML(
            "beforeend",
            `<p class='textReturn'>>>>  ${weatherData.data[input]}</p>`
          );
        } else {
          terminal.insertAdjacentHTML(
            "beforeend",
            `<p class='textError'>>>> no property named  ${input} !</p>`
          );
        }
      }
    }
